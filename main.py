...
class Processor():
    def _init_(self, arg):
        ...
        shutil.copy2(_file_, self.arg.work_dir)
        shutil.copy2('/content/SlowFastSign/SlowFastSign/configs/baseline.yaml', self.arg.work_dir)
        copy_tree('/content/SlowFastSign/SlowFastSign/slowfast_modules', self.arg.work_dir + '/content/SlowFastSign/SlowFastSign/slowfast_modules')
        copy_tree('/content/SlowFastSign/SlowFastSign/modules', self.arg.work_dir + '/content/SlowFastSign/SlowFastSign/modules')
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                if avg_wer < best_avg:
                    best_avg = avg_wer
                    model_path = "{}dev_{:05.2f}test{:05.2f}_epoch{}_model.pt".format(self.arg.work_dir, dev_wer, test_wer, epoch)
                    self.save_model(epoch, model_path)
                if save_model:
